...
<div class="container mt-3">
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <label for="membre">Membre</label><select name='membre' id='membre'>
        <?php
        $connect=mysqli_connect('localhost','root','','bonnebouffe');
        echo "connexion réussie <br>";
        $req1=mysqli_query($connect,"SELECT * FROM MEMBRES");
        if(mysqli_num_rows($req1)>0)
        {
            while($row=mysqli_fetch_assoc($req1))
            {
                echo'<option value="'.$row["idmembre"].'">'.$row["login"].'</option>';
            }        
...
<?php
        session_start();
        if(isset($_POST['nom'])&& isset($_POST['ingredient'])&& isset($_POST['preparation']) && isset($_POST['cout'])&& isset($_POST['date'])&& isset($_POST['membre'])&& isset($_POST['nbpers']))
        {
            $idm=$_POST["membre"];
            echo $idm;
